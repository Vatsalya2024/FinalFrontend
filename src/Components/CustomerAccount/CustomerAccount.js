...
function CustomerAccount() {
    ...
    useEffect(() => {
        const inactiveAccounts = customerAccounts.filter(account => account.status === 'Active' && account.balance === 0);
    ...
    const handleCloseAccount = () => {
        ...
        fetch(`http://localhost:5134/api/CustomerAccount/Close%20Account?accountNumber=${accountNumberToClose}`, {
            method: 'POST',
            headers: {
                'accept': 'text/plain',
                'Authorization': 'Bearer ' + token
    ...
    const handleGetAccountDetails = () => {
        fetch(`http://localhost:5134/api/CustomerAccount/${accountNumber}/${customerId}`, {
            method: 'GET',
            headers: {
                'accept': 'application/json',
                'Authorization': 'Bearer ' + token
    ...
    const handleGetAccountStatement = () => {
        ...
        fetch(`http://localhost:5134/api/CustomerTransaction/AccountStatement?accountNumber=${accountNumber}&startDate=${startDate}&endDate=${endDate}`, {
            method: 'GET',
            headers: {
                'accept': 'application/json',
                'Authorization': 'Bearer ' + token
    ...
    return (
        <div className="container justify-content-center align-items-center mt-lg-5">
            <div className="row">
                    ...
                    <div className="card ">
                        <div className="card-body">
                            ...
                            >
                                <option value="">Select</option>
                                {branches.map(branch => (
                                    <option key={branch.ifscNumber} value={branch.ifscNumber}>{branch.branchName}</option>
                                ))}
                            </select>
                            <br />
                            <button className="btn buttons" onClick={handleOpenAccount}>Open Account</button>
                            <button onClick={handleCancel} className='btn buttons mt-1'>Cancel</button>
    ...
    {customerAccounts
        .map(account => (
            <option key={account.accountNumber} value={account.accountNumber}>{account.accountNumber}</option>
                            ...
                            <button className='btn buttons  mt-1' onClick={handleCancel}>Cancel</button>
    ...
    {customerAccounts
        .map(account => (
            <option key={account.accountNumber} value={account.accountNumber}>{account.accountNumber}</option>
                            ...
                            <button className='btn cancel mt-1' onClick={handleCancel}>Cancel</button>
                            <br />
    ...
    <div className="card mb-2">
        <div className="card-body">
            ...
            />
            <br />
            <button className="btn buttons" onClick={handleGetAccountStatement}>Get Account Statement</button>
            <button className='btn cancel mt-1' onClick={handleCancel}>Cancel</button>
            <br />

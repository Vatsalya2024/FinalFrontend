...
function BankEmployeeAccount() {
    ...
    useEffect(() => {
        const filtered = customers.filter(customer => {
            ...
            return (
                ...
                (typeof phoneNumber === 'string' && phoneNumber.toLowerCase().includes(query))
            );
    ...
    const approvePendingDeletion = (accountNumber) => {
        ...
        fetch(`http://localhost:5134/api/BankEmployeeAccount/ApproveAccountDeletion?accountNumber=${accountNumber}`, {
            method: 'POST',
            headers: {
                'Accept': 'text/plain',
    ...
    const approvePendingAccountCreation = (accountNumber) => {
        ...
        fetch(`http://localhost:5134/api/BankEmployeeAccount/ApproveAccountCreation?accountNumber=${accountNumber}`, {
            method: 'POST',
            headers: {
                'Accept': 'text/plain',
                'Content-Type': 'application/json',
                    ...
                    <div className="card mb-2">
                        <div className="card-body">
                            ...
                            <div className='button-container'>
                                <button className="btn buttons " onClick={handleGetCustomerAccounts}>Get Customer Accounts</button>
                                <button className='btn cancelz ll' onClick={handleCancel}>Close</button>
                            </div>
                            {messages && <p className='alert alert-danger'>{messages}</p>}
                            <br />
                ...
                <div className="col-md-6">
                    <div className="card mt-4">
                        <div className="card-body">
                            ...
                            <div className='button-container'>
                                <button className="btn b mb-2" onClick={getPendingDeletionAccounts}>Get Pending Deletion Accounts</button>
                                <button className='btn cancelz lb' onClick={handleCancel}>Close</button>
                            </div>
                            <ul className="list-group">
                                {pendingDeletionAccounts.map(account => (
                                    <li key={account.accountNumber} className="list-group-item">
                                        ...
                                        <button className="btn b" onClick={() => approvePendingDeletion(account.accountNumber)}>Approve Deletion</button>
                                    </li>
                                ))}
                            </ul>
                        </div>
                ...
                <div className="col-md-6">
                    <div className="card mt-4">
                        <div className="card-body">
                            ...
                            <div className='button-container'>
                                <button className="btn b" onClick={getPendingAccounts}>Get Pending Accounts</button>
                                <button className='btn cancelz lm' onClick={handleCancel}>Close</button>
                            </div>
                            <ul className="list-group">
                                {pendingAccounts.map(account => (
                                    <li key={account.accountNumber} className="list-group-item">
                                        ...
                                        <p>IFSC: {account.ifsc}</p>
                                        <p>CustomerID:{account.customerID}</p>
                                        <button className="btn b" onClick={() => approvePendingAccountCreation(account.accountNumber)}>Approve Creation</button>
                                    </li>
                                ))}
                            </ul>
                        </div>

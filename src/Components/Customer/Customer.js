...
function Customer() {
    ...
    useEffect(() => {
        if (email) {
            fetch(`http://localhost:5134/api/Customer/GetCustomerInfoByEmail?email=${(email)}`)
                .then(response => {
    ...
    const handleResetPassword = () => {
        ...
        fetch(`http://localhost:5134/api/Customer/UpdatePassword?email=${(email)}&newPassword=${(newPassword)}`, {
            method: 'POST',
            headers: {
    ...
    const handleChangePhoneNumber = () => {
        ...
        if (customerInfo) {
            ...
            fetch(`http://localhost:5134/api/Customer/ChangePhoneNumber?id=${(customerId)}&phone=${(phone)}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
    ...
    const handleChangeName = () => {
        ...
        if (customerInfo) {
            ...
            const customerId = customerInfo.customerID;
            fetch(`http://localhost:5134/api/Customer/ChangeName?id=${(customerId)}&name=${(name)}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': 'Bearer ' + token
    ...
    const handleChangeAddress = () => {
        ...
        if (customerInfo) {
            const customerId = customerInfo.customerID;
            fetch(`http://localhost:5134/api/Customer/ChangeAddress?id=${(customerId)}&address=${(address)}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': 'Bearer ' + token
                ...
                <div className="col-md-6">
                    ...
                    <div className="card mt-4">
                        <div className="card-body">
                            ...
                            <button onClick={handleCancel} className="btn cancel mt-1 mx-auto">Cancel</button>
                    ...
                    <div className="card mt-3">
                        <div className="card-body">
                            ...
                            <button onClick={handleChangePhoneNumber} className="btn buttons">Change Phone Number</button>
                             <button onClick={handleCancel} className="btn cancel mt-1">Cancel</button>
                            {message&&<p>{message}</p>}
                            </div>
                        </div>
                        <div className="card mt-3">

...
import '../CustomerBeneficiary/CustomerBeneficiary.css';
...
function CustomerBeneficiary() {
    ...
    const fetchSourceAccounts = () => {
        fetch(`http://localhost:5134/api/CustomerAccount/GetAccountDetailsByCustomerId?customerId=${Customer}`)
            .then(response => response.json())
            .then(data => {
    ...
    const fetchBeneficiaries = () => {
        fetch(`http://localhost:5134/api/CustomerBeneficiaries/GetBeneficiaryByCustomerId?customerId=${Customer}`,{
            headers:{
    ...
    const addBeneficiary = () => {
        const isExistingBeneficiary = beneficiaries.some(beneficiary => beneficiary.beneficiaryAccountNumber === parseInt(beneficiaryAccountNumber));

        const isInvalidAccountNumber = beneficiaryAccountNumber.startsWith('11133');
    
        if (isExistingBeneficiary) {
            setErrorMessage2('Beneficiary with the same account number already exists.');
            return;
        }
        ...
        if(beneficiaryAccountNumber===''||beneficiaryName===''||ifsc===''){
    ...
    const transferToBeneficiary = () => {
        ...
        const requestBody = {
            ...
            beneficiaryAccountNumber: parseInt(selectedBeneficiary.beneficiaryAccountNumber),
            amount: parseInt(transferAmount)
        };

        fetch('http://localhost:5134/api/CustomerBeneficiaries/TransferToBeneficiary', {
            method: 'POST',
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json',
    ...
    const getBranchesByBankName = () => {
        ...
        fetch(`http://localhost:5134/api/CustomerBeneficiaries/GetBankBranchesByBankName?bankName=${selectedBank}`,{
            headers:{
                'Authorization': 'Bearer ' + token
            }
        })
    ...
    const getIFSCByBranchName = () => {
        ...
        fetch(`http://localhost:5134/api/CustomerBeneficiaries/GetIFSCByBranchName?branchName=${branchName}`,{
            headers:{
                'Authorization': 'Bearer ' + token
            }
        })
    ...
    const toggleSelectedBeneficiary = (beneficiary) => {
        if (selectedBeneficiary && selectedBeneficiary.beneficiaryID === beneficiary.beneficiaryID) {
        ...
        <div className="container">
            <div className="row">
                    ...
                    <div className="card h-100 p-3 beneficiary">
                        <div className="card-body">
                            ...
                            <div className="form-group">
                                ...
                                >
                                    ...
                                    {banks.map((bank, index) => (
                                        <option key={index} value={bank.bankName}>{bank.bankName}</option>
                                    ))}
                                </select>
                                <button className="btn" onClick={getBranchesByBankName}>Get Branches</button>
                            ...
                            <div className="form-group">
                                ...
                                <button className="btn" onClick={getIFSCByBranchName}>Get IFSC</button>
                            ...
                            <button className="btn" onClick={addBeneficiary}>Add Beneficiary</button>
                            <div>
                ...
                <div className="card beneficiary">
                    ...
                    <div className="beneficiary-item" key={beneficiary.beneficiaryID}>
                                      
                    <p><strong>Name:</strong> {beneficiary.name}</p>
                    <p><strong>Account Number:</strong> {beneficiary.beneficiaryAccountNumber}</p>
                    <p><strong>IFSC:</strong> {beneficiary.ifsc}</p>
                    <button onClick={() => toggleSelectedBeneficiary(beneficiary)}>
                        {selectedBeneficiary && selectedBeneficiary.beneficiaryID === beneficiary.beneficiaryID ? 'Deselect' : 'Select'}
                    </button>
                    <hr/>
                </div>
            ))}

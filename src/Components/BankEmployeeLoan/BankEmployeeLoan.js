...
function BankEmployeeLoan() {
    ...
    const reviewLoanApplication = () => {
        fetch(`http://localhost:5134/api/BankEmployeeLoan/ReviewLoanApplication/${loanId}`,{
            headers:{
                'Authorization': 'Bearer ' + token
            }
        })
    ...
    const checkCredit = (accountNumber) => {
        fetch(`http://localhost:5134/api/BankEmployeeLoan/check-credit/${accountNumber}`,{
            headers:{
                'Authorization': 'Bearer ' + token
            }
        })
    ...
    const makeLoanDecision = () => {
        ...
        fetch(`http://localhost:5134/api/BankEmployeeLoan/MakeLoanDecision/${loanId}?approved=${approved}`, {
            method: 'POST',
            headers: {
                'accept': 'text/plain',
                'Authorization': 'Bearer ' + token
    ...
    const disburseLoan = () => {
        ...
        .then(data => {
            if (data.status === 'Rejected') {
                ...
                fetch(`http://localhost:5134/api/BankEmployeeLoan/disburse-loan/${loanId}/${disburseAccountNumber}`, {
                    method: 'POST',
                    headers: {
                        'accept': 'text/plain',
                        'Authorization': 'Bearer ' + token
        ...
        <div className="container">
                    ...
                    <ul className="list-group">
                        {filteredLoans.map(loan => (
                            <li key={loan.loanID} className="list-group-item">
                                Loan ID: {loan.loanID}, Loan Amount: {loan.loanAmount}, Status: {loan.status}, Customer ID: {loan.customerID}
                            </li>
                        ))}
                    </ul>
                </div>
            ...
            <div className="card mt-3">
                <div className="card-body">
                    ...
                    <div>
                        <button className="btn x" onClick={reviewLoanApplication}>Review Loan Application</button>
                        <button className='btn cancel hg' onClick={handleCancel}>Close</button>
                    </div>
            ...
            <div className="card mt-3">
                <div className="card-body">
                    ...
                    />
                    <div>
                        <button className="btn x" onClick={makeLoanDecision}>Make Loan Decision</button>
                        <button className='btn cnacel cf' onClick={handleCancel}>Close</button>
                    </div>
                    {message && <p>{message}</p>}
                </div>
            </div>
            ...
            <div className="card mt-3">
                <div className="card-body">
                    ...
                    <div>
                        <button className="btn x" onClick={disburseLoan}>Disburse Loan</button>
                        <button className='btn cancel hd' onClick={handleCancel}>Close</button>
                    </div>
